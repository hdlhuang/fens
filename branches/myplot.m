function h=myplot(p,e,t,p1,v1,p2,v2,p3,v3,p4,v4,p5,v5,p6,v6,p7,v7,p8,v8,p9,v9,p10,v10,p11,v11,p12,v12,p13,v13,p14,v14)
...
%       $Revision: 1.14 $  $Date: 2001/02/09 17:03:18 $
...
for i=1:2:nargs-4,
  ...
  switch lower(Param)
    ...
    case 'shownodelbl'
      shownodelbl = lower(Value);
      if ~isstr(shownodelbl),
        error('shownodelbl must be a string.')
      elseif ~(strcmp(shownodelbl,'off') | strcmp(shownodelbl,'on'))
        error('shownodelbl must be off | on')
      end
    case 'showtrilbl'
      showtrilbl = lower(Value);
      if ~isstr(showtrilbl),
        error('showtrilbl must be a string.')
      elseif ~(strcmp(showtrilbl,'off') | strcmp(showtrilbl,'on'))
        error('showtrilbl must be off | on')
      end
    case 'showbound'
      showbound = lower(Value);
      if ~isstr(showbound),
        error('showbound must be a string.')
      elseif ~(strcmp(showbound,'off') | strcmp(showbound,'on'))
        error('showbound must be off | on')
      end
    otherwise
...
if intool,
  ...
  solver_fig = findobj(get(0,'Children'),'flat','Tag','PDESolver');
  ax = findobj(get(solver_fig,'Children'),'flat','Tag','SolverAxes');
  if isempty(ax),
    error('Intool plot option on. Couldn''t find PDESolver axes.')
  end
end %end if intool
...
if ~plotxy & ~plotz & ~plotflow,
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ax=gca;
  if strcmp(shownodelbl,'on')
    % flag is set: display labels
    % get node data for the current mesh
    limx = get(ax,'Xlim'); limy = get(ax,'Ylim');
    scale=min(diff(limx),diff(limy));
    ind=t(:,1)'; ind=[ind(1:3), ind(1)];
    trilength=max(mean(abs(diff(p(1,ind)))),mean(abs(diff(p(2,ind)))));
    scale = trilength/scale;
    if scale>0.06,      fsize=10;
    elseif scale>0.04,  fsize=8;
    elseif scale>0.02,  fsize=6;
    elseif scale>0.01,  fsize=5;
    elseif scale>0.005, fsize=4;
    else,  fsize=3;     end
    set(ax,'Xlim',limx+[-1 1]*scale*diff(limx));
    set(ax,'Ylim',limy+[-1 1]*scale*diff(limy));
    ...
    for i=1:n,
      txt(i)=text('String',int2str(i),'Units','data',...
        ...
        'FontSize',fsize,'pos',[p(1,i) p(2,i) 1],'color','g',...
        'Parent',ax);
    end
    hh = [hh;txt'];
  end
  
  if strcmp(showtrilbl,'on')
    ...
    for i=1:n,
      ...
      txt(i)=text('String',int2str(i),'Units','data',...
        ...
        'FontSize',fsize,'pos',[xm ym 1],'color','m',...
        'Parent',ax);
    end
    hh = [hh;txt'];
  end
...
if intool,
  ...
  set(solver_fig,'Nextplot','replace')
  if gcf~=solver_fig, set(gcf,'NextPlot','replace'), end
  set(0,'ShowHiddenHandles',showhidd)
else
  hold off
end
